...
[![Github Actions](https://github.com/ctu-vras/robot_body_filter/workflows/CI/badge.svg?branch=ros2)](https://github.com/ctu-vras/robot_body_filter/actions?query=workflow%3ACI)
<!--
...
The basic workings of this filter are done via the [`filters::FilterBase` API](http://wiki.ros.org/filters) implemented
for `sensor_msgs::msg::LaserScan` and `sensor_msgs::msg::PointCloud2` types. This means you can load this filter into a
FilterChain along other filters as usual. Different from the standard filters, this one can also publish several
interesting topics and subscribes to TF.

### General overview

This filter reads robot model and the filter config, subscribes to TF, waits
for data (laser scans or point clouds) and then cleans them from various
artifacts (this is called data filtering).

It can perform 3 kinds of data filters:

- **clip** the data based on the provided sensor limits (parameter `filter/do_clipping`)
- **remove points that are inside or on the surface** of the robot body (parameter `filter/do_contains_test`)
- **remove points that are seen through a part of the robot body** (parameter `filter/do_shadow_test`).

These kinds of tests are further referenced as "clipping", "contains test" and "shadow test".
...
filtered-out points. In laser scans, removal is not an option, so the
...
The fewer links are processed, the better performance. If you're only interested
...
To simplify your model to primitive shapes, you can either manually edit the URDF,
or you can utilize [ColliderGen](https://github.com/cole-bsmr/collidergen).
...
cloud input (although all-at-once laser scans aren't that common). Point-by-point
...
If you do scan deskewing (compensating the motion of the robot during the scan acquisition),
then this filter should be applied before the deskewing (because parts of the robot body are fixed to the lidar and
the robot's motion thus doesn't affect the angle under which the lidar sees the body part).
...
- `scan_point_cloud_no_bsphere` (`sensor_msgs/msg/PointCloud2`)
...
- `scan_point_cloud_no_bbox` (`sensor_msgs/msg/PointCloud2`)
    ...
    Published in filtering frame.
- `scan_point_cloud_no_oriented_bbox` (`sensor_msgs/msg/PointCloud2`)
...
- `scan_point_cloud_no_local_bbox` (`sensor_msgs/msg/PointCloud2`)
    ...
    filtering frame.
- `robot_bounding_sphere` (`robot_body_filter/msg/SphereStamped`) 

    Bounding sphere of the robot body. Turned on by 
    `bounding_sphere.compute` parameter. Published in filtering frame.
- `robot_bounding_box` (`geometry_msgs/msg/PolygonStamped`) 
...
- `robot_oriented_bounding_box` (`robot_body_filter/msg/OrientedBoundingBoxStamped`)
    ...
    frame.
- `robot_local_bounding_box` (`geometry_msgs/msg/PolygonStamped`) 
...
- `~reload_model` (`std_srvs/srv/Trigger`)

    Call this service to re-read the last received URDF model. The usefulness of this in ROS 2 is not confirmed,
    but it has been kept during the migration from ROS 1.
...
- `body_model.robot_description_topic` (`string`, default: `"robot_description"`)

    Name of the topic where the robot model is published.
    **If you migrate from ROS 1, please note that in ROS 1, the model was a parameter and now it is a topic!**
...
These options are there to help correctly set up and debug the filter operation and should be turned off in production
environments since they can degrade performance of the filter.

### Published Topics

- `robot_bounding_sphere_marker` (`visualization_msgs/msg/Marker`) 
...
- `robot_bounding_box_marker` (`visualization_msgs/msg/Marker`) 

    Marker of the bounding box of the robot body. Turned on by 
    `bounding_box.marker` parameter. Published in filtering frame.
- `robot_oriented_bounding_box_marker` (`visualization_msgs/msg/Marker`) 
...
- `robot_local_bounding_box_marker` (`visualization_msgs/msg/Marker`) 
...
- `robot_bounding_sphere_debug` (`visualization_msgs/msg/MarkerArray`)
...
- `robot_bounding_box_debug` (`visualization_msgs/msg/MarkerArray`)
...
- `robot_oriented_bounding_box_debug` (`visualization_msgs/msg/MarkerArray`)
...
- `robot_local_bounding_box_debug` (`visualization_msgs/msg/MarkerArray`)
...
- `robot_model_for_bounding_box` (`visualization_msgs/msg/MarkerArray`)
